    ...
    <AnimatePresence>
      {toolId ? (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="flex items-start justify-between gap-3 border-b border-outline-variant/60 p-5">
              <div className="min-w-0">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-on-surface-variant">
                  Movement history
                </p>
                <h2 className="mt-0.5 break-words text-lg font-extrabold leading-tight text-white">{toolName}</h2>
              </div>
              <button
                type="button"
                aria-label="Close history"
                onClick={onClose}
                className="grid h-9 w-9 shrink-0 cursor-pointer place-items-center rounded-full border border-outline-variant text-on-surface-variant transition-colors hover:bg-surface-container-low hover:text-on-surface"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              {isLoading ? (
                ...
                <p className="text-sm text-on-surface-variant">No movements yet.</p>
              ) : (
                <ol className="space-y-2">
                  {movements.map((m) => (
                    <li key={m.movementId} className="rounded-xl border border-outline-variant/50 px-3 py-2 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-white">{KIND_LABEL[m.kind] ?? m.kind}</span>
                      ...
                      <p className="mt-0.5 text-[11px] text-slate-500">{new Date(m.createdAt).toLocaleString()}</p>
                    </li>
                  ))}
                </ol>
              )}
            ...
            {!isLoading && movements.length > 0 ? (
              <div className="border-t border-outline-variant/60 p-4">
                <button
                  ...
                  className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl border border-error/30 bg-error/10 px-4 py-2.5 text-sm font-semibold text-error transition-colors hover:bg-error/20"
                >
