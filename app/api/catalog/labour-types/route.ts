...
export const POST = withApi(async ({ request, requestId }) => {
  ...
  const existing = await db.select({ id: customLabourTypes.labourTypeId, name: customLabourTypes.name }).from(customLabourTypes);
  const guard = checkDuplicateOrSimilar(name, existing);
  if (guard.blocked) return errorResponse(ERROR_CODES.CONFLICT, "Possible duplicate labour type", 409, { guard }, requestId);

  const inserted = await db.insert(customLabourTypes).values({
    name,
