...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { value } = await getOrSetJson(
      ...
      60,
      async () => {
        // Single round-trip instead of 7 separate queries — each subselect runs in parallel
        // inside Postgres, which is far cheaper than 7 sequential network round-trips.
