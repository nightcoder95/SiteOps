...
export default function SiteDetailPageClient({
  ...
  const [siteResult, setSiteResult] = useState<ClientResult<Site> | null>({
    ok: true,
    data: initialSite,
  });
  ...
  const [sitesResult, setSitesResult] = useState<ClientResult<Site[]> | null>({
    ok: true,
    data: initialSites,
  });
  ...
  async function onSubmitEntry(e: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmitState(res);
    if (res.ok) {
      toastSuccess(`${type} entry submitted`);
      await loadEntries(type);
      form.reset();
      return;
    }
    toastClientError(res);
  ...
  async function onSubmitTransfer(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res.ok) {
      toastSuccess('Transfer request submitted');
      form.reset();
      return;
    }
    toastClientError(res);
  ...
  const [optimisticEntryList, pushOptimisticEntry] = useOptimistic(
    entryList,
    (current, nextEntry: Entry) => [nextEntry, ...current],
  );
