...
type Site = {
  // `siteId` is the uuid every /api/sites/{id}/… route matches on; `id` is the
  // numeric identity column. This page used to key off `id`, which never
  // resolved — fixed while converting the page to SSR.
  id: number;
  siteId: string;
  name: string;
  location: string;
...
export function AdminExpensesPageClient({
  ...
  const [siteId, setSiteId] = useState<string>(initialSites[0]?.siteId ?? '');
  // Seeded from the server render so the site picker is populated in the first
  // paint (audit F16). Expenses still load client-side per selection.
  const { data: sitesResult } = useApiResult<Site[]>('/api/sites', {
    fallbackData: { ok: true, data: initialSites, status: 200 } as ClientResult<Site[]>,
  });
  ...
  useEffect(() => {
    if (!siteId && sitesResult?.ok && sitesResult.data[0]?.siteId) {
      setSiteId(sitesResult.data[0].siteId);
    }
  }, [siteId, sitesResult]);
