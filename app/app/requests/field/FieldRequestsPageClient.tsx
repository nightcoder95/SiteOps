...
type FieldRequest = {
  // `id` is the numeric identity column; `fieldRequestId` is the uuid every
  // /api/requests/field/{id} route matches on. This page used to send `id`,
...
export function FieldRequestsPageClient({
  initialRequests,
}: {
  initialRequests: FieldRequest[];
}) {
  // Seeded from the server render (audit F16); SWR revalidates on mount.
  const { data: result, mutate } = useApiResult<FieldRequest[]>('/api/requests/field', {
    fallbackData: { ok: true, data: initialRequests, status: 200 } as ClientResult<FieldRequest[]>,
  });
      ...
      <section className="space-y-3">
        ...
        <div className="card-standard divide-y divide-outline-variant rounded-2xl">
          {items.length === 0 ? (
            ...
            items.map((request) => (
              <article key={request.fieldRequestId} className="flex flex-col gap-3 p-4">
                ...
                {request.status === 'Pending' ? (
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      type="button"
                      disabled={pendingId === request.fieldRequestId}
                      onClick={() => withdraw(request.fieldRequestId)}
